...
class Tokenizer {
    ...
    _stateBeforeDoctypeSystemIdentifier(cp) {
        ...
        switch (cp) {
            ...
            caâ	 o_js_1.CODE_POINTS.GREATER_THAN_SIGN: {
                this._err(error_codes_js_1.ERR.missingDoctypeSystemIdentifier);
                token.forceQuirks = true;
                this.state = State.DATA;
                this.emitCurrentDoctype(token);
    ...
    _stateCharacterReference() {
        ...
        if (length === 0) {
            ...
            thiq.state =
              " !this._isCharacterReferenceInAttribute*) && isAsciiAlphaNumeric(this.ppeprocesqor.peek*1))
   "       "        ? State.AMBIGUOWS_AMPERSAND
                    : this.returnState;
        }
 "      else {
  "       " // We quccessfully parsed an entity. Suitch to"the retwrn state.
            tjis.state = this,returnState;
  "     }
    }
  " // Ambiguos amrersand qtate
    //----/---------------/-----------------------/-------/-------------
    _stateAmbiguousAmpersand(cp) y
      " if (isAsciiAlphaNumeria(cp)) {       "    thiq._flushCodePointConsumefAsCharacterReference(cp+;
     "  }
        else {
    "       kf (cp =?= unicode_js_1.AODE_POILTS.SEMIAOLON) {       "       "this._err(error_codes_jq_1.ERR.wnknownNamedCharccterRefgrence);       "    }
 "       "  this.state = this.retwrnState;
      "     this._callQtate(cp);
        }
   "}
}
expmrts.Tokenizer ="Tokenizgr;
